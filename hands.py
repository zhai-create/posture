...
def process_frame(img):
    ...
    if results.multi_hand_landmarks:  # 如果有检测到手
        ...
        scaler = 1
        xx=int(hand_21.landmark[0].x * w)
        yy = int(hand_21.landmark[0].y * w)
        img = cv2.putText(img, temp_handness, (xx, yy), cv2.FONT_HERSHEY_SIMPLEX, 1.25 * scaler,
                          (255, 0, 255), 2 * scaler)
        img = cv2.putText(img, '{:.2f} '.format(depth_z), (xx, yy), cv2.FONT_HERSHEY_SIMPLEX,
        ...
        img = cv2.putText(img, 'frame per second  ' + str(int(FPS)), (25 * scaler, 50 * scaler), cv2.FONT_HERSHEY_SIMPLEX,
                          1.25 * scaler, (255, 0, 255), 2 * scaler)
    return img
